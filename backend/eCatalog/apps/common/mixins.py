...
from drf_yasg.utils import no_body, guess_response_status
...
class CustomAutoSchema(SwaggerAutoSchema):
    ...
    def get_default_responses(self):
        ...
        if default_schema:
            if self.has_list_response() or getattr(self.view, "paginate", True):
                default_schema = openapi.Schema(type=openapi.TYPE_ARRAY, items=default_schema)
            if self.should_page() or getattr(self.view, "paginate", True):
